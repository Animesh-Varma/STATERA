#!/bin/bash
echo "[>] STATERA 1K ABLATION SUITE INITIALIZING (30-Epoch Fast Track)..."
...
python train.py --dataset_path ../sim/1K-ablation.hdf5 --decoder_type deconv --temporal_mixer conv1d --target_type dot --backbone vjepa --epochs 30 --funnel_epochs 6 --accumulate_steps 3 --metrics_file 1k_ablation_metrics.json --wandb_name Run-05-Standard-Sigma-Only --temperature 2.0 --finetune_blocks 2

# 7. Run-06-Standard-Softmax-T1 (Testing against Reviewer #2's claims via T=1 Quantization Check)
python train.py --dataset_path ../sim/1K-ablation.hdf5 --decoder_type deconv --temporal_mixer conv1d --target_type crescent --backbone vjepa --epochs 30 --funnel_epochs 6 --accumulate_steps 3 --metrics_file 1k_ablation_metrics.json --wandb_name Run-06-Standard-Softmax-T1 --temperature 1.0 --finetune_blocks 2

# 8. Run-07-Single-Task-Spatial (Testing dual-head 3D auxiliary loss implications on pure 2D trajectory tracking)
python train.py --dataset_path ../sim/1K-ablation.hdf5 --decoder_type deconv --temporal_mixer conv1d --target_type crescent --backbone vjepa --epochs 30 --funnel_epochs 6 --accumulate_steps 3 --single_task --metrics_file 1k_ablation_metrics.json --wandb_name Run-07-Single-Task-Spatial --temperature 2.0 --finetune_blocks 2
